...
import instructionsMap from './instructionsContent';

// Configuración de iconos y colores para cada test
const testConfig = {
  bat7: { icon: 'fas fa-clipboard-list', color: 'text-purple-600' },
  verbal: { icon: 'fas fa-comments', color: 'text-blue-600' },
  espacial: { icon: 'fas fa-cube', color: 'text-indigo-600' },
  atencion: { icon: 'fas fa-eye', color: 'text-red-600' },
  razonamiento: { icon: 'fas fa-puzzle-piece', color: 'text-amber-600' },
  numerico: { icon: 'fas fa-calculator', color: 'text-teal-600' },
  mecanico: { icon: 'fas fa-cogs', color: 'text-slate-600' },
  ortografia: { icon: 'fas fa-spell-check', color: 'text-green-600' }
};
...
const Instructions = () => {
  ...
  useEffect(() => {
    // Carga las instrucciones específicas según el testId
    const fetchData = async () => {
      try {
        // Simular tiempo de carga
        await new Promise(resolve => setTimeout(resolve, 800));

        // Obtener instrucciones del mapa según el testId
        if (instructionsMap[testId]) {
          setTest(instructionsMap[testId]);
        } else {
          toast.warning(`No se encontraron instrucciones para el test: ${testId}`);
        }
  ...
  const handleStartTest = () => {
    ...
    // Navegar al test específico
    if (test.type === 'battery') {
      // Si es la batería completa, redirigir al primer subtest
      if (test.subtests && test.subtests.length > 0) {
        navigate(`/test/${test.subtests[0].id}`);
      } else {
        navigate('/student/tests');
      }
    } else {
      // Si es un test individual, navegar directamente a él
      navigate(`/test/${test.id}`);
    }
  };

  const config = testConfig[testId] || { icon: 'fas fa-clipboard-list', color: 'text-gray-600' };
            ...
            <CardBody>
              <div className="space-y-3">
                {test.instructions.map((instruction, index) => {
                  // Si la instrucción está vacía, renderizar un espacio
                  if (instruction === '') {
                    return <div key={index} className="h-4"></div>;
                  }

                  // Si la instrucción empieza con **, es un título
                  if (instruction.startsWith('**') && instruction.endsWith('**')) {
                    return (
                      <h4 key={index} className="text-lg font-semibold text-gray-800 mt-6 mb-3 border-b border-gray-200 pb-2">
                        {instruction.replace(/\*\*/g, '')}
                      </h4>
                    );
                  }

                  // Contar solo las instrucciones que no son títulos ni espacios vacíos
                  const instructionNumber = test.instructions.slice(0, index + 1)
                    .filter(inst => inst !== '' && !inst.startsWith('**')).length;
